...
class NotificationController extends GetxController
  ...
  void _connectStream() {
    ...
    _sub = NotificationService.streamNotifications().listen((msg) {
      if (notifications.any((n) => n.id == msg.id)) {
        print('[NotificationController] stream skipped duplicate ${msg.id}');
        return;
      }
  ...
  Future<void> openAttachment(NotificationMessage msg) async {
    try {
      if (msg.fileUrl != null && msg.fileUrl!.isNotEmpty) {
        final resolvedUrl = NotificationService.ensureAbsoluteUrl(msg.fileUrl!);
        final uri = Uri.parse(resolvedUrl);
        print('[NotificationController] downloading $resolvedUrl');
  ...
  void _loadLocal() {
    ...
    notifications.sort(
        (a, b) => b.timestampUtc.compareTo(a.timestampUtc));
    ...
    if (_lastTimestamp == null && notifications.isNotEmpty) {
