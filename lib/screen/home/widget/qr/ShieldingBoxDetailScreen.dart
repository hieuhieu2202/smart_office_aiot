...
class ShieldingBoxDetailScreen extends StatelessWidget {
  ...
  String _title() {
    ...
    if (m == '-' && s == '-') return 'Sheilding-Box Detail';
    if (m == '-') return 'Sheilding-Box - $s';
    if (s == '-') return '$m - Sheilding-Box';
    return '$m - $s';
  }
  ...
  String _apiFileName() {
    ...
    debugPrint('[Shielding] API File_Name (raw)  = "$raw"');
    debugPrint('[Shielding] API File_Name (trim) = "$clean"');
    return clean;
  }
  ...
  void _openLogFile(BuildContext context) {
    ...
    Navigator.push(
      context,
      MaterialPageRoute(builder: (_) => PdfViewerScreen(url: url, title: file)),
    );
  }
