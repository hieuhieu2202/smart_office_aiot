...
class _CompactCellChip extends StatelessWidget {
  const _CompactCellChip({
    required this.detail,
  ...
  @override
  Widget build(BuildContext context) {
    final highlighted = highlightCells.contains(
      buildRetestCellKey(detail.modelName, detail.groupName, columnIndex),
    );
    final value = detail.retestRate;
    final rateText = value == null ? 'N/A' : '${value.toStringAsFixed(2)}%';
    final rateColor = value == null
        ? Colors.white54
        : (value >= 5
    ...
    final targetWidth = (availableWidth - 64).clamp(130.0, 200.0) as double;

    return GestureDetector(
      onTap: onTap == null ? null : () => onTap!(detail),
      child: AnimatedContainer(
        duration: const Duration(milliseconds: 250),
        width: math.min(180.0, targetWidth),
        padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 12),
        decoration: BoxDecoration(
          color: _kCompactChipBackground,
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              children: [
                _CompactShiftBadge(label: detail.shiftLabel),
...
class _CompactShiftBadge extends StatelessWidget {
  const _CompactShiftBadge({required this.label});
