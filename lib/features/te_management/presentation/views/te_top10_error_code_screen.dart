...
double _resolveViewportHeight(BuildContext context, SizingInformation sizing) {
  final candidate = sizing.screenSize.height;
...
class _TETop10ErrorCodeScreenState extends State<TETop10ErrorCodeScreen> {
                  ...
                  Positioned.fill(
                    child: SafeArea(
                      child: Padding(
                        ...
                        child: _buildContent(context, sizing),
                      ),
                    ),
                  ),
                  _buildFilterScrim(),
  ...
  Widget _buildContent(BuildContext context, SizingInformation sizing) {
    return Obx(() {
      ...
      final viewportHeight = _resolveViewportHeight(context, sizing);
      if (isWide) {
      ...
      final distributionHeight = _clampDimension(
        viewportHeight * (isTablet ? 0.34 : 0.3),
        220,
        isTablet ? 360 : 300,
      );
      final trendHeight = _clampDimension(
        viewportHeight * (isTablet ? 0.42 : 0.5),
        300,
        isTablet ? 440 : 480,
      );
  ...
  Widget _buildTopErrorTablePanel({required bool isWide, double? height}) {
    final panel = Obx(() {
      ...
      child: Stack(
        ...
        children: [
          ...
          SfCartesianChart(
            ...
            backgroundColor: Colors.transparent,
            margin: const EdgeInsets.only(top: 12, right: 16, left: 10, bottom: 12),
            plotAreaBorderWidth: 0,
            legend: Legend(
              isVisible: true,
            ...
            onSelectionChanged: (args) {
              ...
              if (index == null || index < 0 || index >= configs.length) {
                return;
              }
              _controller.focusErrorTrend(configs[index].error);
            },
            onLegendTapped: (args) {
              final index = args.seriesIndex;
              if (index == null || index < 0 || index >= configs.length) {
            ...
            primaryYAxis: NumericAxis(
              ...
              majorGridLines: const MajorGridLines(color: Color(0x33163B63)),
            ),
            series: <CartesianSeries<dynamic, dynamic>>[
              for (var i = 0; i < configs.length; i++)
                SplineSeries<TETopErrorTrendPointEntity, String>(
                  ...
                  name: configs[i].error.errorCode,
                  dataSource: configs[i].points,
