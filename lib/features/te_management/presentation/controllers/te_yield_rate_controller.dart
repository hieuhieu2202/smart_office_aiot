...
class TEYieldRateController extends GetxController {
  TEYieldRateController({
    required this.getYieldRateReportUseCase,
  ...
  final GetYieldRateReportUseCase getYieldRateReportUseCase;
  ...
  final Rx<TEYieldDetailEntity> detail = TEYieldDetailEntity.empty().obs;
  ...
  TEYieldDetailEntity get filteredDetail {
  ...
  Future<void> fetchReport({bool showLoading = true}) async {
    ...
    try {
      ...
      errorMessage.value = '';
      final data = await getYieldRateReportUseCase(
      ...
      detail.value = TEYieldDetailEntity.empty();
      _setHighlightKeys(const <String>{});
    } finally {
      if (showLoading) {
  ...
  Future<TEErrorDetailEntity?> fetchErrorDetailForCell({
    ...
    debugPrint('[TEYieldRate] GET $apiPath');
  ...
  Set<String> _identifyChangedCells(
    TEYieldDetailEntity previous,
    TEYieldDetailEntity current,
  ) {
    if (!current.hasData) {
      return <String>{};
    }
    ...
    for (final row in current.rows) {
      ...
      for (final groupName in row.groupNames) {
        ...
        final rateValues = row.yieldRate[groupName] ?? const <double?>[];
        final inputValues = row.input[groupName] ?? const <int?>[];
        final firstFailValues = row.firstFail[groupName] ?? const <int?>[];
        final repairQtyValues = row.repairQty[groupName] ?? const <int?>[];
        ...
        for (var index = 0; index < totalColumns; index++) {
          ...
          final repairQty = _valueAt(repairQtyValues, index);
          ...
          if (previousMetrics == null) {
            ...
            if (_doubleChanged(rate, previousMetrics.rateAt(index))) {
              changed = true;
                ...
                repairQty != previousMetrics.repairQtyAt(index) ||
          ...
          if (changed) {
            result.add(buildYieldCellKey(row.modelName, groupName, index));
          }
        }
      }
    }
  ...
  Map<String, Map<String, _YieldGroupSnapshot>> _buildSnapshot(
    TEYieldDetailEntity detail,
  ) {
    final result = <String, Map<String, _YieldGroupSnapshot>>{};
    if (!detail.hasData) {
      return result;
    }

    for (final row in detail.rows) {
      final groups = <String, _YieldGroupSnapshot>{};
      for (final groupName in row.groupNames) {
        groups[groupName] = _YieldGroupSnapshot(
          yieldRate:
              List<double?>.of(row.yieldRate[groupName] ?? const <double?>[]),
          input: List<int?>.of(row.input[groupName] ?? const <int?>[]),
          firstFail: List<int?>.of(row.firstFail[groupName] ?? const <int?>[]),
          repairQty: List<int?>.of(row.repairQty[groupName] ?? const <int?>[]),
...
class _YieldGroupSnapshot {
  ...
  final List<int?> repairQty;
  final List<int?> pass;

  double? rateAt(int index) =>
      index >= 0 && index < yieldRate.length ? yieldRate[index] : null;
  ...
  int? repairQtyAt(int index) =>
      index >= 0 && index < repairQty.length ? repairQty[index] : null;
